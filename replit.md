...
- **Trading Components**:
    ...
    - **Paper Exchange Wrapper (`paper_exchange_wrapper.py`)**: An infrastructure layer that intercepts ccxt calls, routing them to the `PaperTradingSimulator` in PAPER mode and passing them to the real Kraken API in LIVE mode. It manages paper orders and positions with persistence.
