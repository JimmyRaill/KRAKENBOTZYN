...
-   **Discord Notifications**: Alerts via Discord webhook for trade executions, position exits, errors, and daily/weekly performance summaries. Configured via DISCORD_WEBHOOK_URL secret. **Enhanced (Dec 2025)**: Trade entry notifications include mental stop-loss price, take-profit price, and risk:reward ratio. Exit notifications (mental SL/TP triggers) show entry→exit prices, P&L (USD and %), and exit type with emojis ("🛑 Stop-Loss" or "🎯 Take-Profit").
