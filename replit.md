...
- **Self-Learning Components**:
    ...
    - **Trade Result Validator (`trade_result_validator.py`)**: **CRITICAL HARDENING (Nov 15, 2025)**: Expanded error detection to catch ALL Kraken-specific error patterns (EOrder:, EGeneral:, EService:, EFunding:, ETAPI:, "Insufficient funds", "Order could not be created", "Invalid nonce", etc.). Stricter success detection requires EXPLICIT success keywords (BRACKET OK, BUY OK, etc.) AND absence of error patterns. Prevents LLM from claiming successful trades when Kraken errors occur. Structured TradeResult dataclass replaces string-based responses for better validation.
...
- **Trading Components**:
    ...
    - **Commands (`commands.py`)**: Handles manual trading commands for order placement and management. **VERIFIED (Nov 15, 2025)**: Exception handling is intentionally designed to return error strings for the LLM interface. The trade_result_validator.py prevents the LLM from claiming success when these error strings are returned. Dead code files (bot.py, config.py) that created rogue ccxt instances have been deleted.
